...
public class _3_MainClass_복제3_HashMap_addLogOut {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if(loggedInMember != null) {
                ...
                System.out.println("-------------------------------------------");
                // 로그인 성공시, 콘솔창에 3번 로그아웃으로 변경해서 출력
            ...
            } catch (NumberFormatException e) { // 예외발생시 대처하기위한 구문
                System.out.println("숫자만 입력해주세요.");
                // 콘솔에 숫자가 아닌 다른 문자등을 입력시,
                // 반복문이 종료되지 않고, 계속 동작 하게끔 설정하는 작업.
                continue;
            }
            ...
            switch (choice) {
                ...
                case 3:
                    if(loggedInMember != null) {   // 로그인 된 상태
                        loggedInMember = null;     // 로그인 정보 초기화 = 로그아웃
                        System.out.println("로그아웃 되었습니다.");
                    } else {                       // 로그인 안된 상태
                        System.out.println("\n====로그인===== ");
                        // 이메일, 패스워드 정보를 입력 받고,
                        // 배열에 저장된 이메일로 가입된 회원을 찾고, 패스워드 비교 후, 일치하면 로그인.
                        // 아니면, 로그인 불가.
                        System.out.println("이메일 : ");
