...
public class _3_MainClass {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if(loggedInMember != null) { // 로그인 상태라면 - 3번 로그아웃 뜨도록 설정
                System.out.println("-------------------------------------------");
            ...
            } else { // 로그인 안한 상태라면 - 3번 로그인 뜨도록 설정
            ...
            int choice; // choice 정수 변수 선언
            try {
                choice = Integer.parseInt(sc.nextLine()); // 콘솔에 입력받은 값은 문자열이라, 숫자로 변환해줘야함.
            } catch (NumberFormatException e) { // 예외 발생시
                System.out.println("숫자만 입력해주세요.");
                continue;
            }

            switch (choice) { // choice변수에 입력받을 값은 문자열 -> 위에서 숫자로 변환해주는 작업 완료.
                case 1: // 회원 가입.   // 위에서 숫자로 변환작업 안했다면, 여기서 "1" 이렇게 해야함.
...
//                                break;
                ...
                case 4 :
                    ...
                    // 콘솔에서 입력한 숫자가 넘어올 때 문자열로 넘어옴. 참고!
                    ...
                    switch (choiceNumber) {
                        // 입력 받은 숫자가 문자열 타입이르모, "1", "2", "3", "4" 로 표기한다.
                        case "1":
                            ...
                            // 객체 쩜을 찍고 사용 -> ex_ loggedInMember.setPassword (변경할 내용)
                        ...
                        case "3":
                            ...
                            // 입력중에 실수로 숫자가 아닌 다른 문자를 입력할 수도 있다.
                ...
                case 5:
                    ...
                    if(searchType.equals("1")) {
                        // 이메일로 검색
                        // HashMap에서  Key로 바로 검색 가능
                        System.out.println("검색할 이메일 입력: ");
                        String serchEmail = sc.nextLine();

                        if(members.containsKey(serchEmail)) {
                            System.out.println("\n 검색 결과");
                            members.get(serchEmail).showInfo();
                        } else {
                        ...
                        // 상태변수, 검색 되었는지 여부
                        boolean isFound = false; // 기본은 발견 안된 걸로
