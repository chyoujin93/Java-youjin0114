        ...
        while (true) {
            ...
            switch (choice) {
                case 1: // 회원 가입.
                    ...
                    members.put(email, newMember);  // 이메일을 key로 회원객체 저장
                    newMember.join();   // 회원객체 가입완료
                    saveMembers(members);   // 지금까지의 멤버들 파일 저장
                    break;
                ...
                case 2: // 목록조회
                    if (members.isEmpty()) {
                        ...
                        for (MemberBase member: members.values()) {
                            member.showInfo();
                        }   // 멤버즈 안의 회원들의 값들을 꺼내서 하나씩 출력
                    }
                    break;

                case 3: //로그인,로그아웃 (상태변수 핵심)
                    if(loggedInMember != null) { // 로그인 된 상태이면,
                        ...
                        boolean isLogin = false; // 없어도 프로그램은 돌아감. 연습용, 가독성용 변수임
                ...
                case 4:
                    ...
                    // 콘솔에서 입력한 숫자를 , 넘어올 때, 문자열로 넘어옴. 변환 필수.
